...
from keras.layers import Input, Dropout, Dense, BatchNormalization, Activation, concatenate, Conv1D, Embedding, Flatten, BatchNormalization, GlobalMaxPooling1D, GlobalAveragePooling1D
...
def get_model(X_train, max_vocabulary):
    ...
    #cnn_layer1_3 = Dropout(0.4)(cnn_layer1_3)
    #cnn_layer2_3 = Dropout(0.4)(cnn_layer2_3)
    ...
    # model
    model = Model([name, item_desc, brand_name, cat_1, cat_2, cat_3, item_condition, num_vars], output)
    optimizer = Adam()    
...
for i_epoch in range(epochs):
    ...
    model.fit(X_train, dtrain.price.sample(frac= 0.85, random_state=i_epoch), epochs=1, batch_size=batch_arr[i_epoch]
